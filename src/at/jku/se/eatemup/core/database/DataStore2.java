...
public class DataStore2 implements IDatabaseAPI {
	...
	public DataStore2() {
		...
		connection = "jdbc:sqlserver://" + this.url + ";databaseName="
				...
				+ this.dbPassword + ";";
		try {
			connectionSource = new JdbcConnectionSource(connection);
		...
		try {
			...
			Logger.log("account dao creation failed."
					+ Logger.stringifyException(e));
		}
		try {
			logDao = DaoManager.createDao(connectionSource, LogEntry.class);
		} catch (SQLException e) {
			Logger.log("log dao creation failed."
					+ Logger.stringifyException(e));
		}
		try {
			positionDao = DaoManager
					.createDao(connectionSource, Position.class);
		} catch (SQLException e) {
			Logger.log("position dao creation failed."
					+ Logger.stringifyException(e));
		}
	}
	...
	public void addUserPoints(String userid, int points) {
		...
		if (tmp != null) {
			...
			try {
				...
				Logger.log("account point update failed for " + userid + "."
						+ Logger.stringifyException(e));
			}
		}
	}
	...
	public void createTables() {
		try {
			...
			Logger.log("account table creation failed."
					+ Logger.stringifyException(e));
		}
		try {
			...
			Logger.log("log table creation failed."
					+ Logger.stringifyException(e));
		}
		try {
			...
			Logger.log("goodieLocation table creation failed."
					+ Logger.stringifyException(e));
		}
	}
	...
	public Account getAccountByUserid(String userid) {
		try {
			...
			Logger.log("account retrieval failed for " + userid + "."
					+ Logger.stringifyException(e));
			return null;
		}
	}
	...
	public Account getAccountByUsername(String username) {
		try {
			QueryBuilder<Account, String> queryBuilder = accountDao
					.queryBuilder();
			queryBuilder.where().eq("username", username).and()
					.eq("type", AccountType.Standard);
			Account temp = queryBuilder.queryForFirst();
			return temp;
		} catch (SQLException e) {
			Logger.log("account retrieval failed for " + username + "."
					+ Logger.stringifyException(e));
			return null;
		}
	}
	...
	public Account getFacebookAccount(String facebookId) {
		try {
			QueryBuilder<Account, String> queryBuilder = accountDao
					.queryBuilder();
			queryBuilder.where().eq("facebookId", facebookId).and()
					.eq("type", AccountType.Facebook);
			Account temp = queryBuilder.queryForFirst();
			return temp;
		} catch (SQLException e) {
			Logger.log("account retrieval failed for (facebook) " + facebookId
					+ "." + Logger.stringifyException(e));
			return null;
		}
	}
	...
	public ArrayList<GoodiePoint> getGoodiePoints() {
		...
		try {
			...
			Logger.log("retrieve goodie point positions failed."
					+ Logger.stringifyException(e));
			return ret;
		}
	}
	...
	public ArrayList<Account> getHighscore(int topX) {
		...
		try {
			...
			Logger.log("preparing highscore query failed."
					+ Logger.stringifyException(e));
		}
		try {
	...
	public ArrayList<LogEntry> getLogEntries(Date start, Date end) {
		...
		try {
			...
			Logger.log("adding lowerthanequal to logentry query failed."
					+ Logger.stringifyException(e1));
		}
		try {
			queryBuilder.where().ge("created", start);
		} catch (SQLException e1) {
			Logger.log("adding greaterthanequal to logentry query failed."
					+ Logger.stringifyException(e1));
		}
		...
		try {
			...
			Logger.log("preparing logentry query failed."
					+ Logger.stringifyException(e));
		}
		try {
