...
public class DbOperations {
	...
	public int selectUserPoints(String _user)	{
		...
		if(this.ds.connectToDB()){
			this.sqlQuery = "SELECT points FROM [sepr].[sepruser].[user] WHERE nickname LIKE'" + _user + "'";
			try {
	...
	public boolean updateUserPoints(String _user, int points){
		if(this.ds.connectToDB()){
			this.sqlQuery = "UPDATE [sepruser].[user] SET [points] = " + points + "where [user].[nickname] LIKE'" + _user + "'";
			try {
				this.stmt = this.ds.con.createStatement();
	...
	public boolean insertUser(String _user){
		...
		if(this.ds.connectToDB()){
			this.sqlQuery = "INSERT INTO [sepruser].[user]([nickname],[points]) values('" + _user +"',0)";
			try {
				this.stmt = this.ds.con.createStatement();			
				this.stmt.executeUpdate(this.sqlQuery);
				this.stmt.close();
	...
	public int selectGoodiePoints(String _goodieTyp){
		int goodiePoints = 0;
		if(this.ds.connectToDB()){
			this.sqlQuery = "SELECT points FROM [sepr].[sepruser].[GoodiePoints] WHERE [type] LIKE'" + _goodieTyp + "'";
			try {
				this.stmt = this.ds.con.createStatement();
				ResultSet rs = this.stmt.executeQuery(this.sqlQuery);
				while(rs.next()){
